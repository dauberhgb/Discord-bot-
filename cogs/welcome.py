...
# Image decoding is CPU and memory intensive.  Keep join bursts from exhausting
# the process while still allowing normal welcomes to run concurrently.
            ...
            try:
                async with aiohttp.ClientSession(timeout=HTTP_TIMEOUT) as session:
                    async with session.get(bg_url) as resp:
                        if resp.status == 200 and (resp.content_length is None or resp.content_length <= MAX_IMAGE_BYTES):
