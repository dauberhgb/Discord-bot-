...
class ShiftControlView(discord.ui.View):
  ...
  async def pause_btn(
    ...
    if shift_key not in active_shifts:
      msg = (
    ...
    shift_data = active_shifts[shift_key]
  ...
  async def end_btn(
    ...
    shift_data = active_shifts.pop(shift_key)
...
class StaffShiftCog(commands.Cog):
  ...
  async def on_voice_state_update(
    ...
    user_id = member.id
    shift_key = (member.guild.id, user_id)
    if shift_key not in active_shifts:
      return
