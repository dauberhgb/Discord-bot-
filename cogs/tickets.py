...
def is_ticket_owner(channel, user_id):
    """Ticket channel names are created by this cog and end with the owner ID."""
    return channel.name.rsplit("-", 1)[-1] == str(user_id)


def can_manage_ticket(interaction, support_role):
    return (
        is_ticket_owner(interaction.channel, interaction.user.id)
...
class TicketSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if cooldown_key in ticket_cooldowns and current_time - ticket_cooldowns[cooldown_key] < 5:
            msg = "Please wait a few seconds." if lang == "en" else "يرجى الانتظار بضع ثوانٍ قبل محاولة فتح تذكرة أخرى."
            await interaction.followup.send(msg, ephemeral=True)
            return
        ticket_cooldowns[cooldown_key] = current_time
...
class RatingModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        settings = database.get_settings(guild.id)
        support_role_id = settings.get("ticket_support_role")
        support_role = guild.get_role(int(support_role_id)) if str(support_role_id).isdigit() else None
        if not can_manage_ticket(interaction, support_role):
            msg = "You cannot close this ticket." if lang == "en" else "لا تملك صلاحية إغلاق هذه التذكرة."
            await interaction.followup.send(msg, ephemeral=True)
            return
        if self.stars.value not in {"1", "2", "3", "4", "5"}:
            msg = "Rating must be from 1 to 5." if lang == "en" else "يجب أن يكون التقييم من 1 إلى 5."
            await interaction.followup.send(msg, ephemeral=True)
            return
...
class TicketControlView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        support_role_id = settings.get("ticket_support_role")
        support_role = interaction.guild.get_role(int(support_role_id)) if str(support_role_id).isdigit() else None
        if not can_manage_ticket(interaction, support_role):
            lang = settings.get("language", "ar")
            msg = "You cannot close this ticket." if lang == "en" else "لا تملك صلاحية إغلاق هذه التذكرة."
