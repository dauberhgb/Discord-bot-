...
class NetworkCog(commands.Cog):
    ...
    async def network_create(self, ctx, *, network_name: str):
        ...
        db.create_network(network_id, network_name[:100], str(ctx.author.id))
    ...
    async def network_delete(self, ctx, network_id: str = None):
        ...
        if str(ctx.author.id) != str(network.get("owner_id")):
            msg = "🚫 Only the network owner can delete it." if lang == "en" else "🚫 حذف الشبكة مخصص لمالكها فقط."
            await ctx.send(msg)
            return
    ...
    async def network_broadcast(self, ctx, *, message_content: str):
        ...
        for g_data in all_network_guilds:
            ...
            try:
                ...
                self._remember_synced_message(ctx.message.id, target_guild.id, sent_msg.id)
    ...
    async def on_message(self, message):
        ...
        self.processed_messages = {k: v for k, v in self.processed_messages.items() if current_time - v < 300}
        ...
        for g_data in all_network_guilds:
            ...
            try:
                ...
                webhook = self.webhook_cache.get(target_channel.id)
                if webhook is None:
                ...
                # Discord already rate-limits requests; reject oversized relay
                # payloads before downloading them repeatedly for every guild.
                if any(attachment.size > 8 * 1024 * 1024 for attachment in message.attachments):
                    continue
                ...
                self._remember_synced_message(message.id, target_guild.id, sent_msg.id)
            except Exception as e:
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        original_msg_id = self.message_origins.get(payload.message_id, payload.message_id)
        if original_msg_id not in self.synced_messages:
            return
        ...
        for g_data in all_network_guilds:
            ...
            try:
                target_msg_id = self.synced_messages.get(original_msg_id, {}).get(target_guild.id)
    ...
    def _remember_synced_message(self, original_id, guild_id, sent_id):
        """Keep a bounded, correct mapping for reaction mirroring."""
        if len(self.synced_messages) >= 5000:
            self.synced_messages.clear()
            self.message_origins.clear()
            self.reaction_logs.clear()
        self.synced_messages.setdefault(original_id, {})[guild_id] = sent_id
        self.message_origins[sent_id] = original_id
